...
def show_example_image(index=15, training_data=None, training_labels=None, classifications=None):
    ...
    print ("y = " + str(training_labels[0,index]) + ". It's a " + classifications[training_labels[0,index]].decode("utf-8") +  " picture.")
...
def flatten_image_data(original_images=None):
    ...
    flattened_image_features = flattened_data/255.
...
def sample_external_image(num_pixels, parameters, classifications, model):
    ...
    my_image = np.array(PIL.Image.fromarray(image).resize((num_pixels, num_pixels))).reshape((num_pixels*num_pixels*3,1))
...
def train(configs=None):
    ...
    # Train the model
    parameters = model.model(train_x, train_y, layers_dims, num_iterations = 2500, print_cost = True)
