...
from utils.configuration import InvalidConfigFileTypeError, retrieve_configs


def test_n_layer_args_happy_case():

    config_file = "feed_forward/image_classification/tests/resources/n_layer_happy.yaml"
...
def test_invalid_config_filetype():
    config_file = "feed_forward/image_classification/tests/resources/not_a_yaml"
    vars = retrieve_configs(config_file)
    assert vars is None


def test_retrieve_configs_invalid_yaml():
    config_file = "feed_forward/image_classification/tests/resources/invalid.yaml"
    with pytest.raises(InvalidConfigFileTypeError):
        retrieve_configs(config_file)
